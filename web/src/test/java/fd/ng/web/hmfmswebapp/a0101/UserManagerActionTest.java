...
//		String responseValue = post(getActionUrl("bean"), requestParameter);
	...
	public void beanName() {
		Person beanName = new Person();
		beanName.setName("p-name-lr");
		beanName.setAge(99);
		beanName.setSex("0-男");
		beanName.setFavors(new String[]{"p-fa-xxx", "p-fa-yyy"});
		beanName.setAmt(new BigDecimal("33.12"));
		/**
		 * Person 必须是 FeedBean 的儿子、孙子，重孙子，我们自动生成的实体都是FeedBean的重孙子
		 * 如果beanName的属性中有name，不能为addData单独在设置,否者后台是取不到name，而是把实体中的name替换掉了
		 * 如果beanName中有个属性favors，是数组，您在单独赋予这个值，数组的长度会加1，
		 * 注意：其实就一个原则，单独的参数名不能和实体的属性名称一致
		 * */
		String responseValue = new HttpClient()
				//.addData("name", "aaa")
				.addData("favors", "p-cc-fffffff")
				.addData("ppp", beanName)
				.addData("fff", "aaa")
				.post(getActionUrl("beanName"))
				.getBodyString();
		String ar = JsonUtil.getNodeValue(responseValue, "data");
		Person person = JsonUtil.toObjectSafety(ar, Person.class).get();
		System.out.println(person);
		assertThat(person.getName(), is("p-name-lr"));
		assertThat(person.getAmt(), is(new BigDecimal("33.12")));
	}
