...
public class HttpClient {
	...
	public HttpClient addData(String name, FeedBean bean) {
		Field[] fields = bean.getClass().getDeclaredFields();//获取实体类的所有属性，返回Field数组
		try {
			for (Field field : fields) { //遍历所有属性
				Param propAnno = field.getAnnotation(Param.class);

				String nameField = field.getName(); //获取属性的名字
				String nameFieldUpp = nameField.substring(0, 1).toUpperCase() + nameField.substring(1); //将属性的首字符大写，方便构造get，set方法
				Class<?> type = field.getType();
				Method method = null;
				try {
					method = bean.getClass().getMethod("get" + nameFieldUpp);
				...
				if (String.class.isAssignableFrom(type)) addData(nameField,String.valueOf(invoke));
				else if (String[].class.isAssignableFrom(type)) addData(nameField,(String[]) invoke);
				else if (int.class.isAssignableFrom(type)) addData(nameField,(Integer) invoke);
				else if (Integer.class.isAssignableFrom(type)) addData(nameField,(Integer) invoke);
				else if (Integer[].class.isAssignableFrom(type)) addData(nameField,(int[]) invoke);
				else if (int[].class.isAssignableFrom(type)) addData(nameField,(int[]) invoke);

				else if (long.class.isAssignableFrom(type)) addData(nameField,(Long) invoke);
				else if (Long.class.isAssignableFrom(type)) addData(nameField,(Long) invoke);
				else if (Long[].class.isAssignableFrom(type)) addData(nameField,(long[]) invoke);
				else if (long[].class.isAssignableFrom(type)) addData(nameField,(long[]) invoke);
				else if (BigDecimal.class.isAssignableFrom(type)) addData(nameField, String.valueOf(invoke));
				else if (BigDecimal[].class.isAssignableFrom(type)) addData(nameField,(String[]) invoke);
				else{
					throw new IllegalArgumentException("buzhichideleixing");
				}
			}
		}catch (Exception e){
			e.printStackTrace();
	...
	public SubmitMediaType getMediaType() {
